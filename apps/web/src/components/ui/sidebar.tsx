...
// Safe cookie handling utility with Cookie Store API fallback
function setSidebarCookie(name: string, value: string, maxAge: number): void {
	try {
		...
		if ("cookieStore" in window) {
			...
			cookieStore
				...
				.catch((error: Error) => {
					console.warn(
						"Failed to set sidebar cookie via Cookie Store API:",
						error,
					);
				});
			return;
		...
		// Fallback to traditional method with lint suppression
		const cookieString = `${name}=${value}; path=/; max-age=${maxAge}`;
		// biome-ignore lint/suspicious/noDocumentCookie: Safe cookie utility with error handling
		document.cookie = cookieString;
	} catch (error) {
		// Silently handle cookie setting errors (e.g., in environments where cookies are disabled)
