...
app.delete("/upload/realm-icon/:realmId", async (c) => {
	try {
		const realmId = c.req.param("realmId");
...
app.post("/upload/character-image/:characterId", async (c) => {
	try {
...
app.delete("/upload/character-image/:characterId", async (c) => {
	try {
		const characterId = c.req.param("characterId");
...
// Favicon is handled by Next.js static file serving
